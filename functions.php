...
function RFDataEncrypt($data)
{
    $salt=RFSalt();
    $ops=$salt.$data.$salt;
    return base64_encode($ops);        
}
function RFDataDecrypt($data)
{
    $salt=RFSalt();
    $datanew=base64_decode($data);
    $ops=str_replace($salt,"",$datanew);
    return $ops;            
}
function MonthYearDropDown()
{
    $monthArray = range(1, 12);
    if($_POST['month']):$month1=$_POST['month'];else:$month1=date('m');endif;
    if($_POST['year']):$year1=$_POST['year'];else:$year1=date('Y');endif;
    $output.='<form name="mfrm" method="post"><table border="0" width="100%" class="mnthyrs" cellpadding="6" cellspacing="6"><tr><td class="mntflt" width="40%" align="right">Filter by : </td><td><select name="month">';
    $output.='<option value="">Select Month</option>';
    foreach ($monthArray as $month):
        $monthPadding = str_pad($month, 2, "0", STR_PAD_LEFT);
        $fdate = date("F", strtotime("2015-$monthPadding-01"));
        if($month1==$monthPadding):$sltmt='selected="selected"';else:$sltmt='';endif;
        $output.='<option value="'.$monthPadding.'" '.$sltmt.'>'.$fdate.'</option>';
    endforeach;
    $output.='</select>&nbsp;&nbsp;';
    ...
    foreach ($yearArray as $year):
       if($year1==$year):$selected='selected="selected"';else:$selected='';endif;    
        $output.='<option '.$selected.' value="'.$year.'">'.$year.'</option>';
    endforeach; 
    $output.='</select><input type="submit" value="Filter" id="filterpush" class="btn btn-warning actbtn"></td></tr></table></form>';
    return $output;
}
